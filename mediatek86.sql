...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : dim. 21 juin 2026 à 17:14
...
('C0006', '2026-04-01', '10002'),
('C0008', '2027-03-19', '10007');
...
('C0001', '2026-03-14', 45.9),
('C0002', '2026-03-14', 25.5),
('C0003', '2026-03-16', 25),
('C0004', '2026-03-16', 10),
('C0005', '2026-03-16', 5),
('C0006', '2026-03-18', 100),
('C0007', '2026-03-19', 50),
('C0008', '2026-03-19', 100);
...
('C0004', 2, '20003', '00003'),
('C0005', 1, '20003', '00003'),
('C0007', 10, '20003', '00001');

--
-- Déclencheurs `commandedocument`
--
DROP TRIGGER IF EXISTS `after_commandedocument_livree`;
DELIMITER $$
CREATE TRIGGER `after_commandedocument_livree` AFTER UPDATE ON `commandedocument` FOR EACH ROW BEGIN
  DECLARE v_date_commande DATE DEFAULT NULL;
  DECLARE v_nb INT DEFAULT 0;
  DECLARE v_i INT DEFAULT 0;
  DECLARE v_next_numero INT DEFAULT 0;

  IF OLD.idSuivi <> '00003' AND NEW.idSuivi = '00003' THEN

    SELECT c.dateCommande INTO v_date_commande
    FROM commande c
    WHERE c.id = NEW.id
    LIMIT 1;

    IF v_date_commande IS NOT NULL THEN
      SET v_nb = NEW.nbExemplaire;

      SELECT COALESCE(MAX(e.numero), 0) INTO v_next_numero
      FROM exemplaire e
      WHERE e.id = NEW.idLivreDvd;
      ...
      WHILE v_i < v_nb DO
        ...
        VALUES (NEW.idLivreDvd, v_next_numero, v_date_commande, '', '00001');
...
('00017', 'Catastrophes au Brésil', '', 'BD001', '00004', '10018'),
...
('20005', 'Test', '', 'DF001', '00002', '10013'),
('R999', 'Revue mofif', '', 'BD001', '00001', '10018');
...
('00007', 1, '2026-03-14', '', '00004'),
('00007', 2, '2026-03-14', '', '00001'),
('00007', 3, '2026-03-14', '', '00001'),
...
('20003', 1, '2026-03-16', '', '00002'),
('20003', 2, '2026-03-16', '', '00001'),
('20003', 3, '2026-03-16', '', '00001');
...
CREATE TABLE IF NOT EXISTS `service` (
  ...
  `accesDocuments` tinyint(1) NOT NULL DEFAULT '0',
  `accesCommandes` tinyint(1) NOT NULL DEFAULT '0',
  `accesExemplaires` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `service_libelle_unique` (`libelle`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `service` (`id`, `libelle`, `accesDocuments`, `accesCommandes`, `accesExemplaires`) VALUES
('CULT', 'Culture', 0, 0, 0),
('SVC01', 'Direction', 1, 1, 1),
('SVC02', 'Presse', 1, 1, 0);
...
CREATE TABLE IF NOT EXISTS `utilisateur` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `utilisateur_login_unique` (`login`),
...
INSERT INTO `utilisateur` (`id`, `login`, `motDePasse`, `idService`, `actif`) VALUES
('U0001', 'tout', 'ToutPwd_1', 'SVC01', 1),
('U0002', 'partiel', 'PartielPwd_1', 'SVC02', 1),
('U0003', 'culture', 'CulturePwd_1', 'CULT', 1);
...
ALTER TABLE `utilisateur`
  ADD CONSTRAINT `utilisateur_ibfk_1` FOREIGN KEY (`idService`) REFERENCES `service` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
COMMIT;
